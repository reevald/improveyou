<script>
  ...
  onMount(() => {
    ...
    const wrapLengthElem = document.getElementById('wrapLengthBox');
    ...
    wrapLengthElem.classList.remove('hidden');
    wrapLengthElem.classList.add('flex');

    trackTimeElem.innerText = '00:00';
    const audTick = new AudioHandler({
...
<div id="wrapActivityBox" class="relative max-h-screen pb-40 bg-amber-50 text-gray-600">
  ...
  >
    ...
    <div class="text-lg font-oswald font-semibold text-gray-700 mb-2">Length</div>
    <div
      id="wrapLengthBox"
      class="hidden flex-row items-center justify-center w-full shrink-0 mb-6"
    >
      {#each data?.iyData?.resources?.data?.target?.values as value, index}
        <button
          on:click={() => timePickerByIdx(index)}
          class="flex flex-col items-center justify-center h-16 w-16 rounded-xl {index ==
          timePickedIdx
            ? 'bg-amber-500'
            : 'bg-amber-400 hover:bg-amber-500'}  text-amber-50 {index == 0
            ? ''
            : 'ml-2'} cursor-pointer"
        >
          <div class="text-2xl font-bold -mb-1.5">{value / 60}</div>
          <div class="text-md">min</div>
        </button>
      {/each}
    </div>
